...
public class PController implements UltrasonicController {
	...
	private static final double NEAR_RIGHT_MOTOR_SPEED_SCALE = 4; //2,4
	private static final double NEAR_LEFT_MOTOR_SPEED_SCALE = 4.6; //2.3
	private static final double FAR_RIGHT_MOTOR_SPEED_SCALE = 0.65; //0.65
	private static final double FAR_LEFT_MOTOR_SPEED_SCALE = 1;
	...
	public void processUSData(int distance) {
		...
		//		else if (distError > 0 && distError <= 6) {
		//			WallFollowingLab.leftMotor.setSpeed(90);
		//			WallFollowingLab.rightMotor.setSpeed(130);
		//			WallFollowingLab.leftMotor.backward();
		//			WallFollowingLab.rightMotor.backward();
		//		}

		else if (distError > 0 && distError < 21400) {
		...
		//Case 3: If Robot is too far away from the wall
			...
			diff = calcProp(distError);
			WallFollowingLab.leftMotor.setSpeed(MOTOR_SPEED - (int)FAR_LEFT_MOTOR_SPEED_SCALE* diff); //(int) 0.8 *
	...
	public int calcProp(int dist) {
		...
		if (dist > 0) {
			correction = (int)(PROPCONST *(double)dist); //2*
		}
